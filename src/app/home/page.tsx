...
const PostCard = ({ item }: { item: Post }) => {
  const { data: author, loading: authorLoading } = useUserById(item.authorId);
  
  if (authorLoading || !author || !author.image) {
    return <PostSkeleton />;
  }
...
const PostSkeleton = () => (
    <div className="relative h-[70vh] rounded-3xl overflow-hidden shadow-lg max-w-md mx-auto bg-muted">
      ...
      <div className="flex flex-col bg-background">
        ...
        <div className="pl-4 md:pl-6">
          <div className="flex space-x-4 overflow-x-auto pb-2">
            {userLoading && <Skeleton className="w-20 h-20 rounded-full flex-shrink-0" />}
            {myStoryImage && (
            ...
            {storiesLoading && Array.from({length: 5}).map((_, i) => <Skeleton key={i} className="w-20 h-20 rounded-full flex-shrink-0" />)}
            {!storiesLoading && stories.slice(0, 7).map((story) => {
              if (!story.image || story.id === loggedInUser?.id) return null;
        ...
        <main className="px-4 py-4 md:px-6">
          <Tabs defaultValue="make-friends" className="w-full">
            ...
            <TabsContent value="make-friends" className="mt-6 space-y-6">
              {feedLoading && Array.from({length: 2}).map((_, i) => <PostSkeleton key={i} />) }
